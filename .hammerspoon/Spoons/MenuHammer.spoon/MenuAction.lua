...
----------------------------------------------------------------------------------------------------
-- Constructor
function MenuAction.new(desc, command, parentMenu)
...
function MenuAction:getUserInput(valueIdentifier, messageValue, informativeText, defaultValue)
	...
	local buttonValue, textValue = hs.dialog.textPrompt(messageValue, informText, default, "Ok", "Cancel")
...
function MenuAction:runScript(scriptName, useAdmin)
	local scriptFunction = function()
		...
		local scriptTask = hs.task.new("/usr/bin/osascript", function(exitCode, standardOut, standardError)
...
function MenuAction:runShellCommand(shellCommand)
	...
	local shellCommandTask = hs.task.new("/usr/bin/osascript", function(exitCode, standardOut, standardError)
		print("Editor exit code: " .. exitCode)
		if exitCode ~= 0 then
			hs.notify.show("FAILED to execute shell command", shellCommand .. " not found.  Check the logs.", "")
		end
...
function MenuAction:openFile(filePath)
	...
	local openFileTask = hs.task.new("/usr/bin/osascript", function(exitCode, standardOut, standardError)
		print("Exit code: " .. exitCode)
		if exitCode ~= 0 then
			hs.notify.show("FAILED to open file", filePath .. " not found.  Check the logs.", "")
		end
...
function MenuAction:runSystemAction(systemAction, confirm)
	...
	if confirm and not self:getUserConfirmation("Action: " .. systemAction, "Are you sure?") then
		runAction = false
	end
	...
	if runAction then
		local systemActions = {
			[cons.sys.shutdown] = function()
				hs.caffeinate.shutdownSystem()
			end,
			[cons.sys.restart] = function()
				hs.caffeinate.restartSystem()
			end,
			[cons.sys.logout] = function()
				hs.caffeinate.logOut()
			end,
			[cons.sys.logoutnow] = function()
				self:runKeyCommand({ "cmd", "alt", "shift" }, "q")
			end,
			[cons.sys.lockscreen] = function()
				hs.caffeinate.systemSleep()
			end,
			[cons.sys.switchuser] = function()
				hs.caffeinate.lockScreen()
			end,
			[cons.sys.screensaver] = function()
				hs.caffeinate.startScreensaver()
			end,
			[cons.sys.forcequit] = function()
...
function MenuAction:runMediaCommand(action, quantity)
	local mediaCommands = {
		...
		setVolume = function()
			...
			local newVolume = tonumber(self:replacePlaceholders(quantity, self.parentMenu.menuManager.storedValues))
		...
		setBrightness = function()
			local newBrightness = tonumber(self:replacePlaceholders(quantity, self.parentMenu.menuManager.storedValues))
...
function MenuAction:replacePlaceholders(stringToProcess, storedValues, encode)
	...
	for valueIdentifier in string.gmatch(processedString, matchPattern) do
		...
		processedString = string.gsub(processedString, matchString, storedValue)
	end
...
function MenuAction:urlEncode(stringToEncode)
	if stringToEncode then
		...
		stringToEncode = stringToEncode:gsub("([^%w %-%_%.%~])", function(c)
...
function MenuAction:getUserConfirmation(message, informativeText)
	print("Getting user confirmation for message: " .. message .. " and text " .. informativeText)

	local buttonPressed = hs.dialog.blockAlert(message, informativeText, "Ok", "Cancel")
